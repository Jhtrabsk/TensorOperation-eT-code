   subroutine rho_test_qed_ccsd(wf, rho_vovo, cs_vo, ct_vo, ct_vo, d_ov, t_vovo, t_vovo)
!!
!! Generated function
!!
      implicit none
...
!
      real(dp), dimension(wf%n_o_p,wf%n_v), intent(in) :: d_ov
      real(dp), dimension(wf%n_v,wf%n_o_p), intent(in) :: ct_vo
      real(dp), dimension(wf%n_v,wf%n_o), intent(in) :: cs_vo, ct_vo
      real(dp), dimension(wf%n_v,wf%n_o_p,wf%n_v,wf%n_o), intent(in) :: t_vovo
      real(dp), dimension(wf%n_v,wf%n_o,wf%n_v,wf%n_o), intent(in) :: t_vovo
!
      real(dp), dimension(:,:), allocatable :: X1, X2, X3, X4
!
      call mem%alloc(X1, wf%n_v, wf%n_o_p)
      call sort_to_21(d_ov, X1, wf%n_o_p, wf%n_v)
...
!
      call mem%dealloc(X3)
      call mem%alloc(X4, wf%n_o, wf%n_o_p)
!
      call dgemm('T', 'T', &
         wf%n_o, &
         wf%n_o_p, &
